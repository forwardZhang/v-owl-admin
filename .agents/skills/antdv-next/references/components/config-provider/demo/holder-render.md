...
function holderRender(children: any) {
  return h(StyleProvider, { hashPriority: 'high' }, () =>
    h(
      ConfigProvider,
      { componentSize: 'small' },
      {
        default: () =>
          h(App, { message: { maxCount: 1 }, notification: { maxCount: 1 } }, () => children)
      }
    )
  );
}
...
<template>
  <a-space>
    ...
    <a-button type="primary" @click="showNotification"> notification </a-button>
