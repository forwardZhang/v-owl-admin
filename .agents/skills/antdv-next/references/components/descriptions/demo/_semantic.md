...
const items = [{ key: '1', label: 'Telephone', content: '1810000000' }];
</script>

<template>
  <SemanticPreview component-name="Descriptions" :semantics="semantics">
    <template #default="{ classes }">
      <div :style="{ width: '100%', height: '100%' }">
        <a-switch v-model:checked="bordered" />
        Toggle Border
        <a-divider />
        <a-descriptions title="User Info" :items="items" :bordered="bordered" :classes="classes">
          <template #extra>
