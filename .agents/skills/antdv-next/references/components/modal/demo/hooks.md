...
const config = {
  title: 'Use Hook!',
  content: () => [h(ReachableConsumer), h('br'), h(UnreachableConsumer)]
};
...
const UnreachableProvider = defineComponent(
  (props, { slots }) => {
    provide(
      UnreachableContextKey,
      computed(() => props.value)
    );
    return () => {
      return slots?.default?.();
...
<template>
  <ReachableProvider value="Light">
    <a-space>
      <a-button @click="handleClick"> Confirm </a-button>
      <a-button
        @click="
          () => {
