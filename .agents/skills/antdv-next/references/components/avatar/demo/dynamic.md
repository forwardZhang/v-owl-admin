...
<template>
  <div>
    <a-avatar :style="{ backgroundColor: color, verticalAlign: 'middle' }" size="large" :gap="gap">
      {{ user }}
    </a-avatar>
    <a-button size="small" style="margin: 0 16px; vertical-align: middle;" @click="changeUser">
      ChangeUser
    </a-button>
    <a-button size="small" style="vertical-align: middle;" @click="changeGap"> changeGap </a-button>
  </div>
</template>
```
